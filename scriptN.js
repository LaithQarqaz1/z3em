...
(function () {
  ...
  async function fetchState() {
    ...
    try {
      const snap1 = await db.collection('topup').doc('freefireN').get();
      if (snap1.exists) {
    ...
    const snap2 = await db.collection('freefireN').doc('state').get();
    if (snap2.exists) {
      return snap2.data() || {};
    }
...
function applyConfig(cfg) {
  ...
  function listenStateRealtime() {
    ...
    // 1) topup/freefireN.state (JSON string)
    try {
      db.collection('topup').doc('freefireN').onSnapshot(snap => {
        ...
        if (typeof data.state === 'string') {
          try {
            ...
            console.warn('JSON غير صالح في topup/freefireN.state');
          }
        }
      });
    } catch (_) {}

    // 2) freefireN/state (object)
    try {
      db.collection('freefireN').doc('state').onSnapshot(snap => {
        if (snap.exists) {
          const cfg = snap.data() || {};
          applyConfig(cfg);
        }
