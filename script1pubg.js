...
function generateGuardToken(prefix = 'order') {
  var tag = prefix || 'order';
  try {
...
async function getTurnstileTokenInteractive() {
  return generateGuardToken('order');
}
...
function persistOffers(data) {
  try {
    ...
    const wrapped = Object.assign({}, prices, { prices, ts: Date.now(), source: 'pubg' });
...
async function loadPrices(useruid = null, { timeoutMs = 5000, silentOnCached = true } = {}) {
  ...
  try {
    ...
    if (!useruid) return;
    const url = new URL('https://z3em-pubg.laithqarqaz1.workers.dev/');
...
async function sendOrder() {
  ...
  // التحقق من Turnstile قبل الإرسال
    ...
    let total, breakdown;
    try {
      const priceRes = await fetch("https://z3em-pubg.laithqarqaz1.workers.dev/", {
        method: "POST",
  ...
  // ====== Purchase ======
  try {
      const response = await fetch("https://z3em-pubg.laithqarqaz1.workers.dev/", {
        method: "POST",
        headers: {
