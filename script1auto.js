...
async function loadPrices(useruid = null) {
  try {
    const url = new URL("https://freefireauto.qousaistore55.workers.dev/");
...
async function sendOrder() {
  ...
  let total, breakdown;
  try {
    const priceRes = await fetch("https://freefireauto.qousaistore55.workers.dev/", {
      method: "POST",
  ...
  try {
    ...
    const response = await fetch("https://freefireauto.qousaistore55.workers.dev/", {
      method: "POST",
      headers: {
