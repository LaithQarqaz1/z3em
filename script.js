...
(function () {
  ...
  async function fetchState() {
    ...
    try {
      const snap1 = await db.collection('topup').doc('freefire').get();
      if (snap1.exists) {
    ...
    const snap2 = await db.collection('freefire').doc('state').get();
    if (snap2.exists) {
      return snap2.data() || {};
    }
...
function applyConfig(cfg) {
  ...
  function listenStateRealtime() {
    ...
    try {
      db.collection('topup').doc('freefire').onSnapshot(snap => {
        ...
        if (typeof data.state === 'string') {
          try {
            ...
            console.warn('JSON غير صالح في topup/freefire.state');
          }
        }
      });
    } catch (_) {}
    ...
    try {
      db.collection('freefire').doc('state').onSnapshot(snap => {
        if (snap.exists) {
          const cfg = snap.data() || {};
          applyConfig(cfg);
        }
