...
function updateTotalAmount() {
  ...
  if (offers.length) {
    offers.forEach(o => {
      ...
      if (type === "topup") {
        ...
        priceStr = p?.price || "";
        details += `• ${jewels} جوهرة - ${priceStr}<br>`;
      } else {
  ...
  function recalcSelectedAmountUI() {
    ...
    selectedOffers.forEach(offer => {
      ...
      if (price !== undefined) {
        total += price;
        breakdownLines.push(`${offer.jewels ? offer.jewels + " جوهرة" : offer.offerName} : ${price.toFixed(2)} $`);
      } else {
...
(function () {
  ...
  async function fetchState() {
    ...
    try {
      const snap1 = await db.collection('topup').doc('freefireauto').get();
      if (snap1.exists) {
    ...
    const snap2 = await db.collection('freefireauto').doc('state').get();
    if (snap2.exists) {
      return snap2.data() || {};
    }
...
function applyConfig(cfg) {
  ...
  function listenStateRealtime() {
    ...
    // 1) topup/freefireauto.state (JSON string)
    try {
      db.collection('topup').doc('freefireauto').onSnapshot(snap => {
        ...
        if (typeof data.state === 'string') {
          try {
            ...
            console.warn('JSON غير صالح في topup/freefireauto.state');
          }
        }
      });
    } catch (_) {}

    // 2) freefireauto/state (object)
    try {
      db.collection('freefireauto').doc('state').onSnapshot(snap => {
        if (snap.exists) {
          const cfg = snap.data() || {};
          applyConfig(cfg);
        }
